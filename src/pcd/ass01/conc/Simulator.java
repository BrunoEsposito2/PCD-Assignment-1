...
import pcd.ass01.conc.view.Flag;
...
public class Simulator {
	...
	public Simulator(Optional<Controller> controller, Optional<SimulationView> viewer, Optional<Flag> flag) {
		this.viewer = viewer;
	...
	public void execute(long nSteps) {
		...
		long iter = 0;
		this.viewer.get().setSimulationState();
		
		/* simulation loop */
		while (iter < nSteps && this.flag.get().isSet()) {
			...
			if(viewer.isPresent()) viewer.get().updateSimulationScene(bodies, vt, iter, bounds);
		}
		this.flag.get().reset();
		this.viewer.get().setIdleState();
	}
	...
	public MonitorImpl<Body> getMonitor() {
		return this.monitor;
	}
